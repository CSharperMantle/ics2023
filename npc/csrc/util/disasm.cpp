...
void init_disasm(const char *triple) {
  ...
  gIP = target->createMCInstPrinter(
      llvm::Triple(gTriple), AsmInfo->getAssemblerDialect(), *AsmInfo, *gMII, *gMRI);
...
std::string disasm(uint64_t pc, const uint8_t *code, int nbyte) {
  MCInst inst;
  llvm::ArrayRef<uint8_t> arr{code, static_cast<size_t>(nbyte)};
