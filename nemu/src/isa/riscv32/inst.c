...
enum {
  TYPE_R, TYPE_I, TYPE_IS, TYPE_S, TYPE_B, TYPE_U, TYPE_J,
  TYPE_N, // none
};
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // Jump & Link
#ifdef CONFIG_FTRACE
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    ,  J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm; s->isa.is_call = rd == 1 /* ra */);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   ,  I, word_t t = s->pc + 4; s->dnpc = (src1 + imm) & ~(word_t)1; R(rd) = t; s->isa.is_ret = rd == 1 /* ra */);
#else
